...
这四段 prompt 单独看都很朴素，价值在于它们串成了一条“文件驱动”的流水线：每一步的输出，就是下一步的输入文件。真正被复用的不是某段措辞，而是这个**目标/输入/输出/步骤 + 让 AI 提问 + 成果落文件**的骨架——它对写代码成立，对写文档、做设计同样成立，是把一次性提问升级成可复用 Workflow 的通用范式。
