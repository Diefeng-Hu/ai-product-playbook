...
把一个新的第三方服务（登录方式、API 集成、Webhook 回调）接进现有系统之前，先用这个 Prompt 过一遍自检清单，把凭证归属、最小权限、失败排查顺序、同步方式和人机交接点在动手前定下来，而不是等出了问题才补。

已在一次真实的基础设施接入任务中使用：过程中提前发现了权限不足会伪装成“资源不存在”的报错，并在完成后主动提出了凭证归属应该从个人账号迁移到组织账号。不同平台的身份模型不同，清单用于提出问题，不替代平台文档和最小权限验证。
